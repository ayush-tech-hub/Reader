...
void main() {
  group('ListDirectory.sortEntries', () {
    ...
    test('directories always come first', () {
      final sorted =
          ListDirectory.sortEntries(entries, FileSortField.name, true);
    ...
    test('sorts by name case-insensitively', () {
      ...
      expect(
        ...
        ['archive', 'zeta', 'Alpha.txt', 'beta.pdf', 'gamma.zip'],
      );
    });

    test('sorts by size descending', () {
      final sorted =
          ListDirectory.sortEntries(entries, FileSortField.size, false);
    ...
    test('sorts by date ascending', () {
      final sorted =
          ListDirectory.sortEntries(entries, FileSortField.date, true);
      final files = sorted.where((e) => !e.isDirectory).toList();
      expect(
        files.map((e) => e.name).toList(),
        ['gamma.zip', 'beta.pdf', 'Alpha.txt'],
      );
    });
